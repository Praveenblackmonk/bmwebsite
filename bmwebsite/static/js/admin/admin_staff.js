    ...
    $.ajax({
        ...
        success: function(data){
            if(data.success){
                ...
                $('#id_change_status_span_icon_'+id).removeClass('icon-published icon-pending').addClass(data.status_class);
...
function update_plans_count(){
    var url = $('#id_update_plans_url').val();
    $.ajax({
        type: "GET",
        url: url,
        data: 1,
        ...
        success: function(data){
            if(data.success){
                $('#plan-mngt').empty().html(data.append_html);
