...
function filter_content(){
	var dataString='';
    var url= $('#id_listclient_url').val();
    dataString+="sort="+$('#id_sort').val();
    if($('#id_sort_category').val()!='' && $('#id_sort_category').val()!=null){
            dataString+="&category="+$('#id_sort_category').val();
        }
	//adding status to data string
    ...
    if($('#id_search').val()!='' && $('#id_search').val()!=null){
        ...
        dataString+="&search_keyword="+$('#id_search_keyword').val();
	...
	if($('#id_action').val()!='0'){
		...
		if(flag){
			if($('#id_action').val()=='DEL'){
				if(confirm(gettext('Are you sure you want to delete selected post(s) ?'))){flag_action=true;}
			}
			if($('#id_action').val()=='P'){
				if(confirm(gettext('Are you sure you want to Publish selected post(s) ?'))){flag_action=true;}
			}
			if($('#id_action').val()=='R'){
				if(confirm(gettext('Are you sure you want to Reject selected post(s) ?'))){flag_action=true;}
			}
			if($('#id_action').val()=='B'){
				if(confirm(gettext('Are you sure you want to Block selected post(s) ?'))){flag_action=true;}
			}
			if(flag_action){filter_action(0,'#');}
		}else{
			alert('Please select post(s)')
		}
	}
}
